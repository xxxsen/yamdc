...
func CutImageWithFaceRec(img image.Image) (image.Image, error) {
	...
	if img.Bounds().Dx() < img.Bounds().Dy() {
		...
		return cutVerticalImage(img, selectedFace.Min.Y+selectedFace.Dy()/2, defaultAspectRatio)
	} else if img.Bounds().Dx() > img.Bounds().Dy() {
		return cutHorizontalImage(img, selectedFace.Min.X+selectedFace.Dx()/2, defaultAspectRatio)
	} else {
		return cutSquareImage(img, selectedFace.Dx()/2, defaultAspectRatio)
	}
}
