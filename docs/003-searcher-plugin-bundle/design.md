...
1. `category` 为空时归一成 `all`
2. 未命中特定 category 时，只走 `all`
3. 命中特定 category 时：
   1. 先走该 category 链
   2. 再补上 `all` 链
   3. 同名插件去重，保留前面的
...
```text
final_chain = category_chain + all_chain(dedup)
```
...
```json
{
  "searcher_plugin_bundle": {
    "sources": [
      {
        "source_type": "remote",
        "location": "https://github.com/example/plugin-bundle"
      }
    ]
  }
}
...
5. 空 `category` 自动归一成 `all`
6. 同一 manifest 内 `(category, name)` 不允许重复
7. `entry` 目录内插件 `name` 必须唯一
8. `configuration.name` 必须存在于插件集合中
