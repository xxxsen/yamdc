...
export interface MediaLibraryShellMainProps {
  items: MediaLibraryItem[];
  initialStatus: MediaLibraryStatus | null;
}

// MediaLibraryShellMain: 媒体库页面真正的内容编排 - 表格 + 搜索 + 过滤 +
// 分页 + 扫描按钮. 拆出独立文件是为了让顶层 media-library-shell.tsx 只
// 承担 (initialError → ErrorState / Main) 路由切换, 单测可以 100% 覆盖.
// 本文件主体逻辑由真实用户路径 (E2E) + 各子 hook 的单测共同覆盖, 不放进
// vitest coverage include 以避免 happy-path 单测含混过去.
// eslint-disable-next-line max-lines-per-function
export function MediaLibraryShellMain({ items: initialItems, initialStatus }: MediaLibraryShellMainProps) {
      ...
      <section className="panel media-library-overview media-library-overview-wide">
        {!configured ? (
          ...
          <div className="media-library-browser-shell">
            ...
            {dataStale ? (
              <div
                className="app-inline-banner app-inline-banner-warn media-library-stale-banner"
                role="status"
                aria-live="polite"
                data-testid="media-library-stale-banner"
              >
                数据刷新失败，当前显示上一次结果
              </div>
            ) : null}
