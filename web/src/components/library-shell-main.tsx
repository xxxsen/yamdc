...
// 指针/canvas 代码从 /library 首屏 JS 里踢出去.
...
export interface LibraryShellMainProps {
  items: LibraryListItem[];
  initialDetail: LibraryDetail | null;
  initialMediaStatus: MediaLibraryStatus | null;
}

// LibraryShellMain: Library 页真正的内容编排. 拆出独立文件让顶层
// library-shell.tsx 只承担 (initialError → ErrorState / Main) 路由切换.
// 主体逻辑由 useLibraryAssetActions / useLibraryMoveRefresh / utils 单测
// 与 E2E 共同覆盖, 不进 vitest coverage include.
// eslint-disable-next-line complexity, max-lines-per-function
export function LibraryShellMain({ items: initialItems, initialDetail, initialMediaStatus }: LibraryShellMainProps) {
  ...
  return (
    <div className="library-shell">
      {mediaStatusStale ? (
        <div
          className="app-inline-banner app-inline-banner-warn library-shell-stale-banner"
          role="status"
          aria-live="polite"
          data-testid="library-stale-banner"
        >
          数据刷新失败，当前显示上一次结果
        </div>
      ) : null}
