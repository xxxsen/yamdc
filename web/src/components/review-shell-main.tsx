...
export interface ReviewShellMainProps {
  jobs: JobItem[];
  initialScrapeData: ScrapeDataItem | null;
  initialMediaStatus: MediaLibraryStatus | null;
}

// ReviewShellMain: /review 页主体内容编排. 拆出独立文件让顶层 review-shell
// 只承担 (initialError → ErrorState / Main) 路由切换. 主体逻辑由
// useReviewBatchActions / useReviewAssetActions / utils 单测 + E2E 共同
// 覆盖, 不进 vitest coverage include.
// eslint-disable-next-line complexity, max-lines-per-function
export function ReviewShellMain({ jobs, initialScrapeData, initialMediaStatus }: ReviewShellMainProps) {
  ...
  const refreshMediaStatus = useEffectEvent(async () => {
    try {
      ...
      // 后台 polling: 不弹 toast 打扰当前 review 操作, 改置 stale 标记;
      // 任意一次成功 polling 自动清回 false.
  ...
  return (
    <>
      {mediaStatusStale ? (
        <div
          className="app-inline-banner app-inline-banner-warn review-shell-stale-banner"
          role="status"
          aria-live="polite"
