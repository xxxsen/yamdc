...
- 搜索插件：`/home/sen/work/yamdc-plugin`
- 清理脚本：`/home/sen/work/yamdc-script`
...
- 搜索插件：使用 `/home/sen/work/yamdc-plugin/cases/` 里的 case，通过 `yamdc plugin-test` 或等价集成命令验证。
- 清理脚本：使用 `/home/sen/work/yamdc-script/cases/` 与 `/home/sen/work/yamdc-script/rules/`，通过 `yamdc ruleset-test` 或等价集成命令验证。
...
当前仓库尚未提供 `fire-manager` 同等级的 integration-test、Playwright E2E 测试链路。后续新增这些能力时，必须补齐：

- `make integration-test`
- `make e2e-test`

**devcontainer 本轮不新增。** 在 integration / E2E 目标落地前，涉及后端接口、前端关键流程、数据库迁移、Docker 或启动脚本的修改，至少必须执行 `make ci-check`，并在 PR / 交付说明中明确未覆盖的集成或 E2E 风险。

---

## 10. 其他

- 在需求执行完或需要澄清时，需要调用 `mcp-feedback-enhanced`（如果存在该 MCP）询问用户下一步动作。调用超时时间为 1 分钟；如果调用超时，需要重试，最多 60 次。
